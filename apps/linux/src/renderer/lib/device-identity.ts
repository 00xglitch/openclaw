...
export async function loadOrCreateDeviceIdentity(): Promise<DeviceIdentity> {
    ...
    if (raw) {
      ...
      if (
        ...
        if (derivedId !== parsed.deviceId) {
          ...
          await bridge.secretsSet(STORAGE_KEY, JSON.stringify(updated));
          localStorage.removeItem(STORAGE_KEY);
          return { deviceId: derivedId, publicKey: parsed.publicKey, privateKey: parsed.privateKey };
        }
        // Migrate from localStorage to safeStorage if needed
  ...
  await bridge.secretsSet(STORAGE_KEY, JSON.stringify(stored));
  return identity;
}
